...
def call_ollama_api(
    ...
    **kwargs,
    ...
    # 組建請求資料
    payload = {"model": model_name, "prompt": prompt, "temperature": temperature, "num_ctx": num_ctx, **kwargs}
...
def simulate_market_data(json_path):
    ...
    for stock in data:
        # 確保價格在 52周高低之間
        if "price" not in stock or stock["price"] < stock["52_week_low"] or stock["price"] > stock["52_week_high"]:
            stock["price"] = round(random.uniform(stock["52_week_low"], stock["52_week_high"]), 2)

        # 確保變化幅度合理
        if "percent_change" not in stock or abs(stock["percent_change"]) > 20:
