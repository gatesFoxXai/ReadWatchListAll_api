...
class SocketState:
    ...
    def increment(self, isLogin=True):
        ...
        with self._lock:
            if (
                round_num <= EnumLoginStatusType.REQ_SUBSCRIBE_ADD.value
                and round_num >= EnumLoginStatusType.REQ_WatchlistAll.value
            ):
                self.value += 1
